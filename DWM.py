    ...
    def __assert_dir(self):
        if not os.path.exists(self.path):
            ...
            if self.chromedriver_path is None:
                # Only prompt interactively when running as main script
                if __name__ == "__main__":
                    if (input("\n\ndo you want to install chromedriver automatically?? y/n: \t").lower() == "y"):
                        self.__install_chromedriver()
                else:
                    logger.info("running as module, skipping chromedriver prompt")

            else:
                self.__save_token(token=None)
                logger.info("will use specified chromedriver path, no to specify this path again")
...
if __name__ == "__main__":
    ...
    asset_name = tv.get_hist("BAJFINANCE", "NSE", interval=Interval.in_3_hour, n_bars=500, extended_session=False)
    asset_parse = bt.feeds.PandasData(dataname = asset_name, datetime = 0, open=2, high=3, low=4, close=5, volume=6)

        
    #Storage for the OCHL
    open_v = asset_name.open
    open_value = open_v.iloc[-499:]
    
    close_v = asset_name.close
    close_value = close_v.iloc[-499:]
    
    high_v = asset_name.high
    high_value = high_v.iloc[-499:]
    
    low_v = asset_name.low
    low_value = low_v.iloc[-499:]
